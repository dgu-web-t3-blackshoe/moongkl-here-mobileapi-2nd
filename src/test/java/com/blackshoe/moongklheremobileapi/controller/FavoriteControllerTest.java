...
public class FavoriteControllerTest {
    ...
    @Test
    public void getUserFavoritePostList_whenSuccess_returns200() throws Exception {
        //given
        final Page mockPage = new PageImpl(new ArrayList());

        //when
        when(favoriteService.getUserFavoritePostList(any(User.class), any(Integer.class), any(Integer.class))).thenReturn(mockPage);
        final MvcResult mvcResult = mockMvc.perform(
    ...
    @Test
    public void getUserFavoritePostList_whenInvalidUser_returns403() throws Exception {
        //given
        final Page mockPage = new PageImpl(new ArrayList());

        //when
        final MvcResult mvcResult = mockMvc.perform(
                        get("/favorites/{userId}", UUID.randomUUID())
                                .contentType(MediaType.APPLICATION_JSON)
                                .accept(MediaType.APPLICATION_JSON)
                                .with(csrf())
                                .with(user(userDetailService.loadUserByUsername("test"))))
